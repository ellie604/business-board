...
async function clearSupabaseStorage() {
    try {
        ...
        const possibleBuckets = ['message-attachments-dev', 'business-documents-dev'];
...
async function main() {
    ...
    const broker = await prisma.user.create({
        data: {
            ...
            password: "123456",
            role: prisma_preview_1.UserRole.BROKER,
        }
    });
    // Create agents
    const agents = await Promise.all([
        prisma.user.create({
            data: {
                ...
                password: "123456",
                role: prisma_preview_1.UserRole.AGENT,
                managedBy: {
                    connect: {
                        id: broker.id
                    }
    ...
    const sellers = await Promise.all([
        prisma.user.create({
            data: {
                ...
                password: "123456",
                role: prisma_preview_1.UserRole.SELLER,
                managedBy: {
                    connect: {
                        id: agents[0].id
                    }
    ...
    const buyers = await Promise.all([
        prisma.user.create({
            data: {
                ...
                password: "123456",
                role: prisma_preview_1.UserRole.BUYER,
                managedBy: {
                    connect: {
                        id: agents[0].id
                    }
    ...
    await Promise.all(sellers.map(seller => prisma.sellerProgress.create({
        data: {
            sellerId: seller.id,
    ...
    await Promise.all(buyers.map(buyer => prisma.buyerProgress.create({
        data: {
            buyerId: buyer.id,
            currentStep: 0, // Everyone starts at step 0
            completedSteps: [], // No steps completed initially
    ...
    await prisma.message.create({
        data: {
            ...
            senderId: broker.id,
            senderType: prisma_preview_1.UserRole.BROKER,
            senderName: broker.name,
            receiverId: sellers[0].id,
            receiverType: prisma_preview_1.UserRole.SELLER,
    ...
    await prisma.message.create({
        data: {
            ...
            senderType: prisma_preview_1.UserRole.SELLER,
            senderName: sellers[1].name,
            receiverId: agents[0].id,
            receiverType: prisma_preview_1.UserRole.AGENT,
