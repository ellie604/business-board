...
const registerHandler = async (req, res) => {
    try {
        const { name, email, phone, password, role, workingWithRepresentative, interestedInSpecificBusiness, availableFunds, fundsSources, minimumMoneyNeeded, totalPriceWilling, lookingToBuyLocation, address, streetAddress, city, state, zipCode } = req.body;
...
You can view more details about this user in the admin portal.
      `.trim();
            ...
            const messagePromises = brokers.map((broker) => prisma.message.create({
                data: {
                    subject,
                    content,
                    senderId: newUser.id,
            ...
            try {
                for (const message of createdMessages) {
                    await emailService_1.emailService.sendBrokerNotification({
            ...
            const updatePromises = brokers.map((broker) => prisma.user.update({
                where: { id: broker.id },
                data: {
                    unreadCount: {
                        increment: 1
...
function generateFrontendNDAPDFAsync(nda, user, listing) {
...
const ndaSubmitHandler = async (req, res) => {
    try {
        const { firstName, lastName, email, phone, password, organization, addressLine1, addressLine2, city, state, zipCode, listingInterest, // This is now required
        availableMoney, minimumIncome, totalPriceWilling, californiaRegions, timeFrameToPurchase, agreementAccepted, signature } = req.body;
...
${pdfUrl ? `NDA PDF Document: ${pdfUrl}` : 'PDF generation failed - please check with technical support.'}
    `.trim();
        ...
        if (brokers.length > 0) {
            ...
            const messagePromises = brokers.map(async (broker) => {
...
const getAvailableListingsHandler = async (req, res) => {
    try {
...
const contactMessageHandler = async (req, res) => {
    try {
