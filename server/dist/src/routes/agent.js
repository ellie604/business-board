...
const getBuyers = async (req, res, next) => {
    ...
    try {
        ...
        const buyersWithListings = await (0, database_1.getPrisma)().user.findMany({
            where: {
                role: 'BUYER',
                buyingListings: {
                    some: {
...
const checkBuyerStepCompletion = async (buyerId, stepId, listingId) => {
...
const checkBuyerStepCompletionInternal = async (buyerId, stepId, listingId) => {
    ...
    switch (stepId) {
        ...
        case 1: // Email agent - FIXED: Only check if this buyer has selected this specific listing
            ...
            const sentMessages = await (0, database_1.getPrisma)().message.findFirst({
                where: { senderId: buyerId }
            });
            return !!sentMessages;
        case 2: // Fill out NDA - should be tied to specific listing
            if (!listingId)
                return false;
            const ndaDoc = await (0, database_1.getPrisma)().document.findFirst({
                where: {
                    buyerId,
                    stepId: 2,
                    type: 'NDA',
        ...
        case 6: // Download purchase contract - should be tied to specific listing
            if (!listingId)
                return false;
            const purchaseDoc = await (0, database_1.getPrisma)().document.findFirst({
                where: {
                    buyerId,
...
router.get('/listings/:listingId/documents', auth_1.authenticateAgent, async (req, res) => {
    try {
        const { listingId } = req.params;
        const typedReq = req;
        const agentId = typedReq.user?.id;
...
router.get('/listings/:listingId/seller-documents', auth_1.authenticateAgent, async (req, res) => {
    try {
        const { listingId } = req.params;
        const typedReq = req;
        const agentId = typedReq.user?.id;
        ...
        console.log('Agent found seller documents:', documents.map((doc) => ({
            id: doc.id,
...
// agent为listing上传文件
router.post('/listings/:listingId/documents', upload.single('file'), auth_1.authenticateAgent, async (req, res) => {
    try {
        const { listingId } = req.params;
...
// 删除agent上传的文件
router.delete('/listings/:listingId/documents/:documentId', auth_1.authenticateAgent, async (req, res) => {
    try {
...
router.get('/listings', auth_1.authenticateAgent, getListings);
router.get('/sellers', auth_1.authenticateAgent, getSellers);
router.get('/buyers', auth_1.authenticateAgent, getBuyers);
router.get('/listings/:listingId/progress', auth_1.authenticateAgent, getSellerProgress);
router.get('/buyers/:buyerId/listings/:listingId/progress', auth_1.authenticateAgent, getBuyerProgress);
// Get buyer documents for a specific listing from agent perspective
router.get('/buyers/:buyerId/listings/:listingId/documents', auth_1.authenticateAgent, async (req, res) => {
    try {
        const { buyerId, listingId } = req.params;
        const typedReq = req;
        const agentId = typedReq.user?.id;
...
router.get('/listings/:listingId/pre-close-checklist', auth_1.authenticateAgent, async (req, res) => {
    try {
        const { listingId } = req.params;
        const typedReq = req;
        const agentId = typedReq.user?.id;
...
// 更新checklist item
router.put('/listings/:listingId/pre-close-checklist/item', auth_1.authenticateAgent, async (req, res) => {
    try {
        const { listingId } = req.params;
        ...
        // 更新对应的item
        const updatedChecklist = await updateChecklistItem(checklist, categoryId, itemId, agentId, typedReq.user?.name || 'Unknown User', userRole);
        // 合并所有items到一个checklist结构
...
router.get('/buyers/:buyerId/listings/:listingId/due-diligence', auth_1.authenticateAgent, async (req, res) => {
    try {
        const { buyerId, listingId } = req.params;
        const typedReq = req;
        const agentId = typedReq.user?.id;
...
// agent上传尽职调查文档
router.post('/listings/:listingId/due-diligence/upload', upload.single('file'), auth_1.authenticateAgent, async (req, res) => {
    try {
        const { listingId } = req.params;
