...
// 获取所有 listing
router.get('/', auth_1.authenticateBroker, async (req, res, next) => {
    const typedReq = req;
    try {
...
// 新增 listing
router.post('/', auth_1.authenticateBroker, async (req, res, next) => {
    const typedReq = req;
    try {
        if (!typedReq.user) {
            res.status(401).json({ message: 'Authentication required' });
        ...
        // 验证卖家
        let seller = null; // Changed type to any to accommodate privateSeller
        if (sellerId) {
...
// 编辑 listing
router.put('/:id', auth_1.authenticateBroker, async (req, res, next) => {
    try {
        // 在函数内部获取 prisma 实例
        const prisma = (0, database_1.getPrisma)();
...
router.patch('/:id/archive', auth_1.authenticateBroker, async (req, res, next) => {
    try {
...
// 重新激活 listing
router.patch('/:id/reactivate', auth_1.authenticateBroker, async (req, res, next) => {
    try {
        const listingId = req.params.id;
        // 在函数内部获取 prisma 实例
        const prisma = (0, database_1.getPrisma)();
...
router.delete('/:id', auth_1.authenticateBroker, async (req, res, next) => {
    try {
        // 在函数内部获取 prisma 实例
        const prisma = (0, database_1.getPrisma)();
