    ...
    try {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            const isCompleted = checkStepCompletionOptimized(i, {
                ...
                questionnaire,
                preCloseChecklist,
                completedStepsFromDB: sellerProgress.completedSteps
            });
...
// Optimized step completion check using batched data
function checkStepCompletionOptimized(stepId, data) {
    ...
    switch (stepId) {
        ...
        case 3: // Fill questionnaire
            if (!questionnaire)
                return false;
            return documents.some(doc => doc.type === 'QUESTIONNAIRE' &&
                doc.category === 'SELLER_UPLOAD' &&
                doc.stepId === 3 &&
                doc.status === 'COMPLETED');
        ...
        case 7: // Upload due diligence
            return documents.some(doc => doc.stepId === 7 &&
                doc.category === 'SELLER_UPLOAD' &&
                doc.status === 'COMPLETED');
        ...
        case 9: // Download closing docs
            return documents.some(doc => doc.stepId === 9 &&
...
const markStepIncomplete = async (req, res, next) => {
    const typedReq = req;
    try {
        const sellerId = typedReq.user?.id;
        const { stepId } = req.body;
...
const getStepDocuments = async (req, res, next) => {
    ...
    try {
        ...
        const stepRequirement = STEP_DOCUMENT_REQUIREMENTS[stepId];
        res.json({
            documents,
...
router.get('/listings/:listingId/documents', auth_1.authenticateSeller, async (req, res) => {
    try {
...
router.get('/listings/:listingId/broker-documents', auth_1.authenticateSeller, async (req, res) => {
    try {
        const { listingId } = req.params;
        const typedReq = req;
        if (!typedReq.user) {
...
// seller为listing上传文件
router.post('/listings/:listingId/documents', upload.single('file'), auth_1.authenticateSeller, async (req, res) => {
    try {
        const { listingId } = req.params;
...
// seller删除listing的文档
router.delete('/listings/:listingId/documents/:documentId', auth_1.authenticateSeller, async (req, res) => {
    try {
...
// 获取seller的问卷数据
router.get('/questionnaire', auth_1.authenticateSeller, async (req, res) => {
    try {
        const typedReq = req;
        if (!typedReq.user) {
            res.status(401).json({ message: 'Unauthorized' });
...
// 保存问卷（不生成PDF）
router.post('/questionnaire/save', auth_1.authenticateSeller, async (req, res) => {
    try {
        const typedReq = req;
...
// 提交问卷并生成PDF
router.post('/questionnaire/submit', auth_1.authenticateSeller, async (req, res) => {
    try {
        const typedReq = req;
        const { questionnaire } = req.body;
...
// 异步PDF生成函数
function generateQuestionnairePDFAsync(questionnaire, user) {
...
function addQuestion(doc, question, answer) {
    // Add a new page if not enough space
    if (doc.y > 720) {
        doc.addPage();
    }
...
function addStaffingTable(doc, staffingData) {
    doc.addPage();
...
function addWeeklyStaffingTable(doc, weeklyData) {
    doc.addPage();
...
router.get('/listings', auth_1.authenticateSeller, getListings);
router.get('/progress', auth_1.authenticateSeller, getProgress);
router.post('/select-listing', auth_1.authenticateSeller, selectListing);
router.post('/update-step', auth_1.authenticateSeller, updateStep);
router.post('/mark-step-completed', auth_1.authenticateSeller, markStepCompleted);
router.post('/mark-step-incomplete', auth_1.authenticateSeller, markStepIncomplete);
router.get('/step/:stepId/documents', auth_1.authenticateSeller, getStepDocuments);
router.post('/step/:stepId/upload', auth_1.authenticateSeller, uploadStepDocument);
router.post('/step/:stepId/download', auth_1.authenticateSeller, downloadStepDocument);
router.get('/current-listing', auth_1.authenticateSeller, getCurrentListing);
// Pre-Close Checklist APIs
// 获取listing的pre-close checklist
router.get('/listings/:listingId/pre-close-checklist', auth_1.authenticateSeller, async (req, res) => {
    try {
        const { listingId } = req.params;
        const typedReq = req;
        if (!typedReq.user) {
...
// 更新checklist item
router.put('/listings/:listingId/pre-close-checklist/item', auth_1.authenticateSeller, async (req, res) => {
    try {
        const { listingId } = req.params;
